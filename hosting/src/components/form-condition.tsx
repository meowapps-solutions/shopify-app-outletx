...
export default function FormCondition({ condition, onChange }: { condition: Rule['conditions'][0], onChange?: (condition: Rule['conditions'][0]) => void }) {
  const [state, setState] = useSyncedState(condition);
  const AllInOneDataOptions: {
    config: Record<Rule['conditions'][0]['type'], TAllInOneData>,
    toConditionOptions(): ({ label: string, value: string, disabled: boolean | undefined, })[],
    toComparisonOptions(type?: Rule['conditions'][0]['type']): ({ label: string, value: string, })[],
  } = {
    config: {
    ...
    toComparisonOptions(type = state.type) {
      const options: Record<TAllInOneData['type'], { label: string, value: Rule['conditions'][0]['operator'] }[]> = {
        number: [
    ...
    <BlockStack gap='300'>
      <InlineGrid gap='200' columns={2}>
        <Select
          ...
          value={state.type}
          onChange={selected => {
            const type = selected as Rule['conditions'][0]['type'];
            const operator = AllInOneDataOptions.toComparisonOptions(type)[0].value as Rule['conditions'][0]['operator'];
        ...
        <Select
          ...
          onChange={selected => {
            setState(prev => ({ ...prev, operator: selected as Rule['conditions'][0]['operator'] }));
          }}
        />
      </InlineGrid>
        ...
        <TextField
          ...
          connectedLeft={
            <Select
              ...
              value={state.type}
              onChange={(selected) => {
                setState(prev => ({ ...prev, type: selected as Rule['conditions'][0]['type'] }));
              }}
            />
          }
        />
