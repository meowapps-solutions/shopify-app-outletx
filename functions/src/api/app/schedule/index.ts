...
const applyTrigger = async (
  ...
  if (trigger.type === 'discount' || trigger.type === 'discount_fixed_amount') {
    ...
    const compareAtPrice = await (async () => {
      ...
      return response.data.productVariant.compareAtPrice || response.data.productVariant.price;
    })();
...
const updateProductMetafields = async (client: GraphqlClient, syncData: SyncData, triggeredRule: TriggeredRule) => {
  ...
  const newValue: SyncData['triggered_rules'] = [...oldValue, triggeredRule];
  const response = await client.request(
    `#graphql
